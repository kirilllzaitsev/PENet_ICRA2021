...
def load_calib():
    ...
    Proj = np.reshape(np.array([float(p) for p in Proj_str]), (3, 4)).astype(float)
...
class CustomKittiDepth(KittiDepth):
    """
    ...
    def __init__(
        ...
        self.ground_truth_paths = args.ground_truth_paths
    ...
    def get_input_paths(
        ...
        gt_paths = self.ground_truth_paths if self.ground_truth_paths is not None else [None] * len(
            self.image_paths
        )
        return {
