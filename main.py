...
import vis_utils
from dataloaders.kitti_loader import CustomKittiDepth as KittiDepth
from dataloaders.kitti_loader import input_options, load_calib
from meta import base_kitti_depth_dir, base_kitti_raw_dir, base_paper_dir
...
parser.add_argument(
    "--data-folder-save",
    default=f"{base_paper_dir}/submit_test/",
    type=str,
    metavar="PATH",
...
parser.add_argument("--save_prediction", action="store_true", default=False, help="save depths")
...
args.result = os.path.join(base_paper_dir, "results")
...
args.image_paths = data_utils.read_paths(f"{args.dataset_paths_dir}/image.txt")
args.intrinsics_paths = data_utils.read_paths(f"{args.dataset_paths_dir}/intrinsics.txt")
args.sparse_depth_paths = data_utils.read_paths(f"{args.dataset_paths_dir}/sparse_depth.txt")
args.ground_truth_paths = None

take_first_n = 5
if take_first_n is not None:
    args.image_paths = args.image_paths[:take_first_n]
    args.intrinsics_paths = args.intrinsics_paths[:take_first_n]
    args.sparse_depth_paths = args.sparse_depth_paths[:take_first_n]
...
def iterate(mode, args, loader, model, optimizer, logger, epoch):
    ...
    for i, batch_data in enumerate(loader):
        ...
        if mode == "test_completion" or args.save_prediction:
            str_i = str(i)
...
def main():
    ...
    if args.freeze_backbone == True:
        ...
        optimizer = torch.optim.Adam(
            [
                {"params": model_bone_params, "lr": args.lr / 10},
    ...
    for epoch in range(args.start_epoch, args.epochs):
        ...
        iterate(
            "train", args, train_loader, model, optimizer, logger, epoch
        )  # train for one epoch
        ...
        for p in model.parameters():
            p.requires_grad = False
        result, is_best = iterate(
            "val", args, val_loader, model, None, logger, epoch
